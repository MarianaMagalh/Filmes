...
export default function FormStep1({ formData, handleChange, nextStep, errors }) {
    ...
    return (
        <form className='formFilme' onSubmit={(e) => e.preventDefault()}>
            ...
            {/* --- CAMPO POSTER (Corrigido) --- */}
            <label className='labelFilme'>Poster URL</label>
            <input 
                className={`inputFilme ${errors.poster ? 'input-error' : ''}`}
                type="text"
                name="poster"  // <--- ADICIONADO: Essencial para o handleChange
                value={formData.poster} // <--- CORRIGIDO: Estava formData.nomeFilme
            ...
            <input 
                className={`inputFilme ${errors.nomeFilme ? 'input-error' : ''}`}
                type="text"
                name="nomeFilme" // <--- ADICIONADO
            ...
            {/* --- CAMPO DURAÇÃO (Corrigido) --- */}
            <label className='labelFilme'>Tempo de Duração</label>
            <input 
                className={`inputFilme ${errors.duracao ? 'input-error' : ''}`}
                type="text"
                name="duracao" // <--- ADICIONADO
            ...
            <input 
                className={`inputFilme ${errors.anoLancamento ? 'input-error' : ''}`}
                type="text"
                name="anoLancamento" // <--- ADICIONADO
            ...
            {/* --- CAMPO PRODUTORA (Corrigido) --- */}
            <label className='labelFilme'>Produtora</label>
            <input 
                className={`inputFilme ${errors.produtora ? 'input-error' : ''}`}
                type="text"
                name="produtora" // <--- ADICIONADO
            ...
            {/* --- CAMPO DIRETOR (Corrigido) --- */}
            <label className='labelFilme'>Diretor</label>
            <input 
                className={`inputFilme ${errors.diretor ? 'input-error' : ''}`}
                type="text"
                name="diretor" // <--- ADICIONADO
            ...
            <div className='alinhaBotao'>
                {/* Botão cancelar volta para a Home (exemplo) */}
                <Link to="/Home">
                    <button className='btnCance' type="button">Cancelar</button>
                </Link>
                
                {/* Botão Próximo chama a função nextStep */}
